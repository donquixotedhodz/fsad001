...
class DocumentController extends MainController {
    ...
    private function hasColumn($table, $column) {
        ...
        if (array_key_exists($cacheKey, $columnsCache)) {
        ...
        $columnsCache[$cacheKey] = (int)$stmt->fetchColumn() > 0;
    ...
    public function uploadDocument() {
        ...
        try {
            ...
            for ($i = 0; $i < $totalFiles; $i++) {
                if ($files['error'][$i] != 0) {
                    $failureReasons[] = "Upload error (code {$files['error'][$i]}) for file #" . ($i + 1);
                    continue;
                }
                ...
                // Validate file extension first (more reliable than browser MIME)
                $file_extension = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
                if (empty($file_extension) || !in_array($file_extension, $allowed_extensions, true)) {
                    $failureReasons[] = "Unsupported file type for {$file_name}";
                    continue;
                }

                // Validate file type (best effort)
                $detectedMime = '';
                if (function_exists('finfo_open')) {
                    $finfo = finfo_open(FILEINFO_MIME_TYPE);
                    if ($finfo) {
                        $detectedMime = finfo_file($finfo, $file_tmp) ?: '';
                        finfo_close($finfo);
                    }
                }
                $mimeToCheck = $detectedMime ?: $file_type;
                if (!empty($mimeToCheck) && !in_array($mimeToCheck, $allowed_mime_types, true)) {
                    $failureReasons[] = "Invalid MIME type for {$file_name}";
                    continue;
                }

                // Validate file size (max 50MB)
                if ($file_size > 50 * 1024 * 1024) {
                    $failureReasons[] = "File too large for {$file_name}";
                    continue;
                }
                ...
                if (!move_uploaded_file($file_tmp, $file_path)) {
                    $failureReasons[] = "Failed to move uploaded file {$file_name}";
                    continue;
                }
                ...
                for ($idx = 0; $idx < $maxCount; $idx++) {
                    ...
                    // Insert record with schema compatibility (department/team may not exist on older DBs)
                    $insertColumns = ['ec', 'item', 'recommending_approvals', 'approving_authority', 'control_point', 'file_path', 'file_name'];
                    $insertValues = [$ec, $item, $rec_approval, $app_authority, $all_control_points, $relative_path, $file_name];

                    if ($this->hasColumn('manap', 'department')) {
                        $insertColumns[] = 'department';
                        $insertValues[] = $department;
                    }

                    if ($this->hasColumn('manap', 'team')) {
                        $insertColumns[] = 'team';
                        $insertValues[] = $team;
                    }
                    ...
                    try {
                        $stmt = $this->conn->prepare(
                            "INSERT INTO manap (" . implode(', ', $insertColumns) . ") VALUES (" . $placeholders . ")"
                        );
                        $result = $stmt->execute($insertValues);
                    } catch (Exception $e) {
                        $result = false;
                        $failureReasons[] = "Database insert failed for {$file_name}: " . $e->getMessage();
                    }

                    if ($result) {
                        $lastId = $this->conn->lastInsertId();
                        $uploadedCount++;
                        $insertedForCurrentFile++;
                ...
                // If no DB row inserted for this file, clean up moved file
                if ($insertedForCurrentFile === 0 && file_exists($file_path)) {
                    @unlink($file_path);
                }
            }
    ...
    public function getAllDocuments() {
        $stmt = $this->conn->prepare("SELECT * FROM manap ORDER BY id DESC, created_at DESC");
        $stmt->execute();
